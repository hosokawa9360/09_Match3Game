...
https://hosokawa9360.github.io/09_Match3Game/
...
```
### １０．課題　パズトラ風ゲームを作成しよう
画面上部にモンスターを表示させ、3マッチにより攻撃して倒す。
モンスターに配置は、次の５パターン
中央のみ　（小モンスター）　（中モンスター）　（大モンスター）
左、中央、右　（小、中、小）　（小、小、小）

#####（戦闘ロジック）
- 消えたオーブと同じ色のモンスターに攻撃を行う
- 基本攻撃力は10
- 3ブロック以上消すと、（消した個数－３）×0.2×10のボーナスダメージを敵に与える
- 3すくみがある　弱点属性で攻撃をした場合、１．２倍の攻撃ボーナスがつく
  - 火属性は木属性より強い
  - 木属性は水属性より強い
  - 水属性は火属性より強い
- 光属性は回復魔法
  黄色のブロックを消すと、攻撃力の0.1倍の値を回復値としてキャラクターのHPを回復させる。
- 闇属性はスリップダメージ
  紫色のブロックを消した場合、持続的なダメージを敵に与えます。攻撃力の0.05の値をダメージとして、敵のパーティの全キャラクターに攻撃します。継続回数は最大で3回までです。

#####（パーティクル）
  各属性の攻撃を行った場合、パーティクルエフェクトを実装しなさい。  　  

#####（ターン制）
  プレイヤーの攻撃とモンスターからの攻撃は入れ替え制。どちらかのHPが０になったらゲーム終了。

#####（モンスターのデータ）
  小モンスター　　"hp" : 250,"atk" : 20,"coin" : 100～199,"exp" : 10～19
  中モンスター　　"hp" : 500,"atk" : 100,"coin" : 200～299,"exp" : 20～29
  大モンスター　　"hp" : 1000,"atk" : 500,"coin" : 300～500,"exp" : 30～39

####（プレイヤーのデータ）
  "hp" :  1000～2500,		"atk" : 10　"coin" : 0,"exp" : 0

### ライフゲージの実装
プレイヤーおよびモンスターのライフゲージを表示させること

### 勝敗
モンスターパーティと5回戦う。すべてのモンスターを倒すと勝ち。体力回復は光属性の回復魔法のみ。
